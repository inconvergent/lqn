...
(defun preproc/fx-or-expr (d)
  (etypecase d (boolean d) (cons d) (keyword d)
    (symbol (if (all? d) :_ `(,d _))) (sequence d)))

(defun if-equal (a b) (when (equal a b) a))
(defun preproc/xpr-sel (ty k) (declare (symbol k))
  (etypecase ty (number `(if-equal ,k ,ty))
                (keyword `(and (stringp ,k) (isub? ,k ,(ct/kv/key ty))))
                (string `(and (stringp ,k) (sub? ,k ,ty)))
                (symbol `(when (,ty ,k) ,k))
                (cons ty)
                (boolean `(if-equal ,ty ,k))))

(defun preproc/|| (qq)
  (unless qq (error "||lqn: missing args."))
...
(defun preproc/$$ (q &optional (m :+))
  (unless q (error "$$lqn: missing args."))
  (labels
...
(defun preproc/** (q &optional (mm :?))
  (unless q (error "**lqn: missing args."))
  (labels ((unpack- (o) (dsb (m sk) (unpack-mode o mm) `(,m ,(preproc/xpr-sel sk :_)))))
    (let* ((q* (remove-if #'all? q)) (res (mapcar #'unpack- q*)))
      (if (= (length q) (length q*)) res (cons :_ res)))))
...
(defun compile/*map (rec conf d &aux (dat* (gk conf :dat))) ; (*map ...)
  (awg (i ires dat)
    (labels ((do-map (expr)
      ...
      (case (length d) (0 (error "*map: missing args."))
        (1 (let ((cd (car d)))
...
(defun compile/*? (rec conf d &aux (cd (car d)) (sd (second d))) ; (*? test expr) ; filter, map
  ...
  (awg (i ires dat dat* vv)
    `(loop with ,ires of-type vector = (mav) with ,vv of-type vector = (vec! ,(gk conf :dat))
       for ,dat across (vec! ,vv) for ,i from 0
       for ,dat* = ,(funcall rec (dat/new conf dat) (preproc/xpr-sel cd dat))
       do (//fxs/op/ (,dat ,dat* ,i)
            (when ,dat*
              (vex ,ires ,(if (= (length d) 1) dat*
                              (funcall rec (dat/new conf dat*) (preproc/fx-or-expr sd))))))
       finally (return ,ires))))
...
(defun compile/xpr (rec conf d &aux (dat (gk conf :dat)) (cd (butlast d 2))) ; (xpr sel .. hit miss)
  ...
  (labels ((get-modes (&rest mm)
             (loop for (m expr) in (strip-all cd)
     ...
     (let ((ors (get-modes :? :%)) (nots (get-modes :-)) (ands (get-modes :+)))
      `(//fxs/op/ (,(gk conf :par t) ,dat ,(gk conf :i t))
        (if (and (or ,(car- all? cd)
                     ,@(when ors ors) ,@(when ands `((and ,@ands))))
                 ,@(when nots `((not (or ,@nots)))))
          ,@(mapcar (lambda (d) (funcall rec conf (preproc/fx-or-expr d)))
                    (last d 2)))))))
(defun compile/mxpr (rec conf d)
  (awg (f dat)
    (labels ((do-fxtx (d) `(,(funcall rec `((:dat . ,f) (:par . ,dat) (:i . 0))
                               `(?xpr ,@(butlast d 1) t nil))
                            ,(funcall rec `((:dat . ,f) (:par . ,dat) (:i . 0))
                                (preproc/fx-or-expr (car (last d)))))))
      `(let ((,dat ,(gk conf :dat)))
         (m/replfx (,dat ,f) ,@(loop for d* in d collect (do-fxtx d*)))))))
...
(defun proc-qry (conf* q) "compile lqn query"
  (labels
    ((rec (conf d)
       (cond ((all? d) (gk conf :dat))
         ((stringp d) d)
         ((vectorp d) (compile/*map #'rec conf (coerce d 'list)))
         ((atom d) d)
         ((optrig? :||    d) (compile/||   #'rec conf (preproc/|| (cdr d))))
         ((optrig? :**    d) (compile/**   #'rec conf (preproc/** (cdr d))))
         ((optrig? :*$    d) (compile/*$   #'rec conf (preproc/$$ (cdr d))))
         ((optrig? :$$    d) (compile/$$   #'rec conf (preproc/$$ (cdr d))))
         ((optrig? :$*    d) (compile/$*   #'rec conf (preproc/$$ (cdr d))))
         ...
         ((optrig? :?xpr  d) (compile/xpr  #'rec conf
